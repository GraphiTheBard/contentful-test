const client = contentful.createClient({space: "80qcbh7fbkhr",accessToken: "HdS3u9MOfM0ZDHPETdrikIFV4yyXs8lW5vT3fMnUS1k",});

let allBikes = [];let bikeTypes = [];let currentType = null;
...
return new Intl.NumberFormat("en-US", {style: "currency",currency: "INR",minimumFractionDigits: 0,maximumFractionDigits: 0,}).format(price);}
...
if (bikeTypes.length > 0) {currentType = bikeTypes[0];}}
...
const bikesToShow = currentType? allBikes.filter((b) => b.bikeType === currentType): allBikes;
...
imgDiv.innerHTML =
  `<img src="${initialImage}" alt="${bike.bikeName}">`;
...
if (bike.colours && bike.colours.length > 0) {
  ...
  bike.colours.forEach((colorHex, colorIndex) => {
    ...
    swatch.className =
      `color-swatch ${colorIndex === 0 ? "active" : ""}`;
    ...
    swatch.onclick = (event) => {
      ...
      const colorSpecificImage =
        bike.bikeImages.find(
          (img) =>
            img.includes(colorHex.replace("#", "")) ||
            img.includes(
              bikeId + "_" + colorHex.replace("#", "")
            )
        );
